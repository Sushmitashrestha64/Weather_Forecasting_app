...
def weather_view(request):
    ...
    status, result_dict = train_and_save_models(city)
    if status == "failed":
        messages.error(request, f"Model training failed: {result_dict.get('error')}")
        return redirect('index_view')
    ...
    try:
        ...
        context = {
            ...
            'bg_class': map_weather_to_class(current_weather['description']),
